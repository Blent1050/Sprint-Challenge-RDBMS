...
1. Explain the difference between `RDBMS` and `SQL`.
   A RDBMS is a relation database management system. SQL is a structured query language. RDBMS is a model, whereas SQL is a language that follows that modal.
1. Why do tables need a `primary key`?
   This is for row-level accessible and also allows for faster querying. The primary key is also unique.
1. What is the name given to a table column that references the primary key on another table.
   Foreign Key
1. What do we need in order to have a _many to many_ relationship between two tables.
   A junction table relating the two tables.
